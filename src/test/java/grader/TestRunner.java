...
public class TestRunner {
    ...
    public void test() throws NoSuchMethodException, IllegalAccessException {
        ...
        try {
            ...
            if (tc.expectedMsg() == null) {
                msgOk = observedMsg == null;
            } else {
                msgOk = observedMsg != null && observedMsg.contains(tc.expectedMsg());
            }
        }
        if (!exceptionTypeOk) {
        ...
        if (!msgOk) {
            System.err.println(tc.nameAndComment()+": unexpected exception message '" + observedMsg + "', expected: '" + tc.expectedMsg() + "'");
        }
